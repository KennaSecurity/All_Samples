...
while keep_trying
  ...
  query_response = RestClient::Request.execute(
    ...
    headers: @headers
      ...
      log_output << "Unable to get assets - UnprocessableEntity: #{query_url}... (time: #{Time.now.to_s}, start time: #{start_time.to_s})\n"
      log_output.close
      puts "Unable to get assets: #{query_url}"
    rescue URI::InvalidURIError
      log_output = File.open(output_filename,'a+')
      log_output << "Unable to get assets - InvalidURI: #{query_url}... (time: #{Time.now.to_s}, start time: #{start_time.to_s})\n"
      log_output.close
      puts "Unable to get assets: #{query_url}"
  ...
  query_response_json.each do |item|
    ...
    if tag_columns.count > 0 then
      ...
      tag_columns.each{|the_column|
        current_tags.each{|cur_tag|
          if cur_tag.start_with?(the_column) then
            tag_list << cur_tag
          end
        }
      }
    end      # pull other columns that need prefixes'
