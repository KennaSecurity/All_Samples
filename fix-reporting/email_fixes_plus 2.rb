...
CSV.foreach(@csv_file, :headers => true){|row|
  ...
  report_url = "https://api.eu.kennasecurity.com/fixes/search?search_id=#{risk_meter_id}"
  ...
  #puts report_url
    ...
    csv << csv_headers
      ...
      json_data.each do |fix|
        ...
        asset_url = "https://api.eu.kennasecurity.com/assets/search?status\%5B\%5D=active&id\%5B\%5D=#{asset_ids.join("&id\%5B\%5D=")}"
