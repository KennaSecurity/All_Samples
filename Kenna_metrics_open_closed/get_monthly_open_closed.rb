...
def processURL(urlstring)
  ...
  begin
    ...
    log_output << "UnprocessableEntity: #{urlstring}... (time: #{Time.now.to_s}, start time: #{@start_time.to_s})\n"
    log_output.close
    puts "UnprocessableEntity: #{e.message} #{urlstring}"
  rescue RestClient::BadRequest => e
    log_output = File.open(@output_filename,'a+')
    log_output << "BadRequest: #{urlstring}... (time: #{Time.now.to_s}, start time: #{@start_time.to_s})\n"
    log_output.close
    ...
    if @retries < @max_retries
      ...
      log_output = File.open(@output_filename,'a+')
      log_output << "General RestClient error #{urlstring}... (time: #{Time.now.to_s}, start time: #{@start_time.to_s})\n"
      log_output.close
...
CSV.foreach(@csv_file, :headers => true){|row|
  ...
  rm_tag = row[2]
    ...
    if !rm_tag.nil? && !rm_tag.empty? then
      query_url = "#{query_url}tag:%22#{rm_tag}%22+AND+"
    end
