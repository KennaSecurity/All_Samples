  ...
  doc.xpath("//OWASPZAPReport").each do |report|
    ...
    report.xpath("site").each do |node|
      ...
      node.xpath("alerts/alertitem").each do |item|
        ...
        puts "wascid = #{wascid}"
        if wascid.empty? && cweid.empty? then
          next
        end
        ...
        item.xpath("instances/instance").each do |issue|
          ...
          begin     
            ...
            if get_response_json.count > 0 then 
              vuln_id = get_response_json[0]['id']
            end 
            ...
            vuln_update_json = {
              'vulnerability' => {
                ...
                'last_seen_time' => last_seen,
                'custom_fields' => {
                  "#{@riskcode_id}" => riskcode,
                  "#{@confidence_id}" => confidence,
                  "#{@riskdesc_id}" => "#{riskdesc}",
                  "#{@scandata_id}" => "OWASPZAP"
                }
              }
            }
