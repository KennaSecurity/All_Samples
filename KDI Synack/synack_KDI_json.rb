...
base_json.each do |item|
  ...
  #########################
    ...
    created = DateTime.strptime(created,"%Y-%m-%dT%T.%LZ").strftime(date_format_KDI) unless created.nil? || created.empty?

    url.each do |url_asset|
      create_asset(url_asset,application)
      create_asset_vuln(url_asset,scanner_type,scanner_id,last_seen,created,scanner_score.to_i,details,closed,status)
