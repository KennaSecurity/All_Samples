...
CSV.foreach(@csv_file, :headers => true, :encoding => "UTF-8"){|row|
  ...
  query_response_json.each do |item|
    ...
    if !row['Exposure Risk Rating'].nil? then tag_list << "ERR:" + row['Exposure Risk Rating'].strip end
    if !row['DR Tier'].nil? then tag_list << "DR:" + row['DR Tier'].strip end
    if !row['IT Manager First'].nil? then tag_list << "IT MGR:" + row['IT Manager First'].strip + " " + row['IT Manager Last'].strip end
    if !row['IIT Director First'].nil? then tag_list << "IT DIR:" + row['IT Director First'].strip + " " + row['IT Director Last'].strip end
    if !row['Bus Director First'].nil? then tag_list << "BUS DIR:" + row['Bus Director First'].strip + " " + row['Bus Director Last'].strip end
    if !row['VP POC First'].nil? then tag_list << "VP POC:" + row['VP POC First'].strip + " " + row['VP POC Last'].strip end
    if !row['IT VP First'].nil? then tag_list << "IT VP:" + row['IT VP First'].strip + " " + row['IT VP Last'].strip end
