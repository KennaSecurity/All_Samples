...
CSV.parse(File.open(@data_file, 'r:bom|utf-8'){|f| f.read}, :headers => @has_header.eql?('true') ? true : false) do |row|
  ...
  if @assets_only == "false" then # DBro - Added for ASSET ONLY Run 
    ...
    #in vuln section ##  scanner_id =
      ...
      cve_id_removal = cve_id.split(',') unless cve_id_removal.nil?
      cve_id_removal = cve_id_removal.map{|x| x.strip} unless cve_id_removal.nil? # This line will strip all leading whitespaces before the CVE_ID values
      cve_id = cve_id_removal.select{|x| x.start_with? "CVE"}.join(',') unless cve_id_removal.nil? # This line will exclude any non CVE_IDs that doesn't start with CVE
